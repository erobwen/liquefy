...
gitHosts$2.sourcehut = Object.assign({}, defaults, {
...
const url$2 = require$$0__default$2["default"];
...
const parseGitUrl = (giturl) => {
  ...
  try {
    result = new url$2.URL(correctedUrl);
  } catch (err) {}
...
function readPackageSync({cwd, normalize = true} = {}) {
	...
	const json = parseJson_1(fs__default["default"].readFileSync(filePath, 'utf8'));
 ...
 */
function externalToFunction(external, functionName = 'external') {
    if (typeof external === 'function') {
        ...
        return (id) => external.some((module) => module instanceof RegExp ? module.test(id) : module === id);
    }
    ...
    // https://github.com/pmowrer/rollup-plugin-peer-deps-external/issues/29
        ...
        throw new TypeError(`rollup-plugin-bundle-esm: '${functionName}' option must be a function or an array.`);
    }
}

function depsExternal({ builtins = true, dependencies = true, packagePath, peerDependencies = true, } = {}) {
    return {
        name: 'deps-external',
        options(opts) {
            let pkg;
            if (packagePath) {
                if (fs__default["default"].statSync(packagePath).isFile()) {
                    packagePath = path__namespace.dirname(packagePath);
                }
            ...
            if (builtins) {
                packageIds.push(...node_module.builtinModules.flatMap((m) => [m, `node:${m}`]));
            }
            packageIds = packageIds
                .map((packageId) => {
                try {
                    return resolve__namespace.sync(packageId);
                }
                catch {
                    return false;
                }
            ...
            const newExternal = (source, importer, isResolved) => {
                if (originalExternal(source, importer, isResolved))
                    return true;
                /**
                ...
                */
                try {
                    const resolvedPath = resolve__namespace.sync(source);
                    const resolvedDirname = path__namespace.dirname(resolvedPath);
                    return packageIds.some((id) => resolvedDirname.startsWith(path__namespace.dirname(id)));
                }
                catch {
                    return false;
                }
