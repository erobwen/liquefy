 ...
 * rooted at this element, so the child's own DOMTarget writes (lastChild,
 ...
 * abstractions (DOMTarget's reactive appendElement()/reattachElement() vs.
 ...
 * flagOverlapWithMovedPredecessor(), and cascade.dom/src/test/domTarget.js's
...
export class DOMContextContainer extends DOMNodeRenderComponent {
  ...
  render(context) {
    ...
    const u = this.unobservable;
    if (!u.innerContext) {
      u.innerContext = new RenderContext(DOMTarget.forElement(u.element));
    }
