...
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  ...
  function span(...parameters) {
    ...
    return flow_core.getTarget().create({ type: "dom.elementNode", tagName: "span", key: properties.key, classNameOverride: "span", attributes, children: properties.children, animate: properties.animate });
  }
  function div(...parameters) {
    let properties = flow_core.findKeyInProperties(flow_core.readFlowProperties(parameters));
    const attributes = extractAttributes(properties);
    return flow_core.getTarget().create({ type: "dom.elementNode", tagName: "div", key: properties.key, classNameOverride: "div", attributes, children: properties.children, animate: properties.animate });
  }
  function styledDiv(classNameOverride, style, parameters) {
    const properties = flow_core.findKeyInProperties(flow_core.readFlowProperties(parameters));
    const attributes = extractAttributes(properties);
    attributes.style = { ...style, ...attributes.style };
    return flow_core.getTarget().create({ type: "dom.elementNode", key: properties.key, classNameOverride, tagName: "div", attributes, ...properties });
  }
  function textToTextNode(properties) {
    if (properties.text) {
      properties.children = // [
      flow_core.getTarget().create({
        type: "dom.textNode",
  ...
  class ZoomFlyDOMNodeAnimation extends DOMNodeAnimation {
    ...
    addTrailersForMovedAndRemovedBeforeDomBuilding(node) {
      ...
      if (node.leader && node.leader === node.parentNode && node.parentNode.isControlledByAnimation) {
        ...
        flow_core.insertAfter(trailer, node);
      }
      this.hide(trailer);
    }
    ...
    setupALeaderForIncomingWithOriginalFootprint(node) {
      ...
      if (!leader) {
        ...
        flow_core.insertAfter(leader, node);
      }
    ...
    recordBoundsInNewStructure(node) {
      ...
      flow_core.draw(node.newStructureBounds, "red");
    }
  ...
  class DOMNode extends flow_core.FlowPrimitive {
    ...
    ensureDomNodeBuilt() {
      flow_core.finalize(this);
      if (!this.buildDOMRepeater) {
        this.buildDOMRepeater = flow_core.repeat("[" + aggregateToString(this) + "].buildDOMRepeater", (repeater) => {
      ...
      recoveredNodes.forEach((node2) => {
        if (!newChildNodes.includes(node2)) {
          ...
          if (!anchor2) {
            ...
            insertAfter(newChildNodes, anchor2, node2);
          }
        }
      });
    ...
    ensureDomNodeExists() {
      if (this.givenDomNode) {
        ...
        this.createElementRepeater = flow_core.repeat(mostAbstractFlow(this).toString() + ".createElementRepeater", (repeater) => {
          if (flow_core.trace) log$3(repeater.causalityString());
          ...
          if (flow_core.trace) log$3(this.domNode);
        }, { priority: 2 });
      }
      return this.domNode;
    }
  ...
  function installDOMAnimation() {
    ...
    flow_core.configuration.onFinishReBuildingDOMCallbacks.push(onFinishReBuildingDOM);
  }
  ...
  function freezeFlowChanges() {
    count++;
    if (flow_core.traceAnimation && flow_core.traceWarnings) console.warn("Risky to use freeze " + count);
  }
  function unfreezeFlowChanges() {
    count--;
    if (flow_core.traceAnimation && flow_core.traceWarnings) console.warn("Unfreeze... " + count);
  }
  let domFlowTargets = [];
  ...
  function onFinishReBuildingFlow() {
    counter++;
    if (flow_core.traceAnimation) {
      flow_core.logAnimationFrameGroup(counter);
    ...
    flow_core.logAnimationSeparator("---------------------------------------- Measure original bounds... ------------------------------------------");
    for (let flow of flowChanges.allAnimatedFlows()) {
      if (flow.getDomNode()) {
        flow.animation.recordOriginalBoundsAndStyle(flow);
      }
    }
    flow_core.logAnimationSeparator("---------------------------------------- Prepare for DOM building... -----------------------------------------");
    for (let flow of flowChanges.allAnimatedFlows()) {
      if (flow.domNode) {
        flow.animation.prepareForDOMBuilding(flow);
      }
    }
    flow_core.logAnimationSeparator("---------------------------------------- Rebuilding DOM... ----------------------------------------------------");
    if (flow_core.traceAnimation) console.groupCollapsed("...");
  ...
  function onFinishReBuildingDOM() {
    ...
    if (flow_core.traceAnimation) console.groupEnd();
    flow_core.logAnimationSeparator("---------------------------------------- DOM rebuilt, measure target sizes ... -------------------------------");
    for (let flow of flowChanges.allAnimatedFlows()) {
      if (flow.domNode) {
        flow.animation.domJustRebuiltMeasureTargetSizes(flow);
      }
    }
    flow_core.logAnimationSeparator("---------------------------------------- Emulate original footprints and styles ------------------------------");
    for (let flow of flowChanges.allAnimatedFlows()) {
      if (flow.domNode) {
        flow.animation.emulateOriginalFootprintsAndFixateAnimatedStyle(flow);
      }
    }
    flow_core.logAnimationSeparator("---------------------------------------- Emulate original bounds for FLIP animations -------------------------");
    for (let flow of flowChanges.allAnimatedFlows()) {
      if (flow.domNode) {
  ...
  function activateAnimationAfterFirstRender(currentFlowChanges) {
    flow_core.postponeInvalidations();
    requestAnimationFrame(() => {
      flow_core.logAnimationSeparator("---------------------------------------- Rendered first frame, activate animations...  ---------------------");
      ...
      flow_core.logAnimationSeparator("---------------------------------------- Setup animation cleanup...  ---------------------");
      for (let flow of currentFlowChanges.allAnimatedFlows()) {
        if (flow.domNode) {
          flow.animation.setupAnimationCleanup(flow);
        }
      }
      flow_core.logAnimationSeparator(counter + "------------------------------------------------------------------------------------------------------------");
      console.groupEnd();
      flow_core.continueInvalidations();
    });
  }
  ...
  class DOMFlowTarget extends flow_core.FlowTarget {
    constructor(rootElement, configuration = {}) {
      ...
      this.state = flow_core.observable({
        modalDiv: null
      });
      return flow_core.observable(this, this.key);
    }
    toString() {
