...
import { findKeyInProperties, readFlowProperties, getTarget, findTextAndKeyInProperties, findTextAndKeyInPropertiesUsingCase, insertAfter, draw, FlowPrimitive, traceWarnings, finalize, repeat, trace, configuration, traceAnimation, logAnimationFrameGroup, logAnimationSeparator, postponeInvalidations, continueInvalidations, FlowTarget, transaction, observable, workOnPriorityLevel } from "@liquefy/flow.core";
  ...
  ensureDomNodeExists() {
    if (this.givenDomNode) {
      ...
      this.createElementRepeater = repeat(mostAbstractFlow(this).toString() + ".createElementRepeater", (repeater) => {
        if (trace) log$3(repeater.causalityString());
