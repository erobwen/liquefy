import { extractProperty, getFlowProperties, getTarget } from "@liquefy/flow.core";
...
export function findImplicitSingleTextInContent(properties) {
  ...
  if (content.length !== 1) throw new Error("Expected just one implicit content")
  if (!["string", "number"].includes(typeof content[0])) throw new Error("Expecting a string or number as content");
